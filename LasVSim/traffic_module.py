...
class TrafficData:
    ...
    def save_traffic(self, traffic, path):
        ...
        for veh in traffic:
             # VAR_POSITION VAR_ANGLE VAR_LENGTH VAR_WIDTH
            self.file.write(struct.pack('6f', *[traffic[veh][64],
                                                ...
                                                traffic[veh][77]]))
            # print(fmt),
            name_length = len(traffic[veh][79])  #VAR_TYPE
            fmt = 'i'
    ...
    for i in range(len(othercarname)):
        ...
        if name not in othercar_dict:
            othercarinfo.append({'x': 99999, 'y': 99999, 'v': 0, 'angle': 0,
                                 ...
                                 'max_decel': 0, 'current_lane': 0, 'current_road': 0})
            continue
        car = {}
        ...
        # print('--------')
        # print('othercar_dict[name]', othercar_dict[name])
        car['current_road'] = othercar_dict[name][traci.constants.VAR_ROAD_ID ]
        # print('roadID', othercar_dict[name][traci.constants.VAR_ROAD_ID])
        # print('--------')
...
class Traffic(object):
    ...
    def __init__(self, step_length, path=None, traffic_type=None,
        ...
        if traffic_type == 'No Traffic':
            ...
            if init_traffic is not None:
                ...
                traffic_data.save_traffic(self.random_traffic, './LasVSim/Scenario/Highway_endtoend')
                # traffic_data.save_traffic(self.random_traffic, 'C:/Users/yangs/Desktop/dsac_ramp_for_changan/LasVSim/Scenario/Ramp2Highway_endtoend')
                RANDOM_TRAFFIC = copy.deepcopy(self.random_traffic)
                HISTORY_TRAFFIC_SETTING = [traffic_type, traffic_density, path]
                self.traffic_change_flag = True
            else:
    ...
    def get_vehicles(self):
        ...
        for i in range(VEHICLE_COUNT):
            # print('vehraodid',  other_veh_info[i]['current_road'])
            ...
            if self.vehicles[i] is not None:
                ...
                wt = self.sim_time
            # 超出自车视野的车辆不进行渲染
            if math.fabs(ego_x - c_x) > 600 or math.fabs(ego_y - c_y) > 600:
                render_flag = False
            else:
                render_flag = True
            self.vehicles[i] = dict(type=c_t, x=c_x, y=c_y, v=c_v, angle=c_a,
                                    ...
                                    lane_index=other_veh_info[i][
                                        'current_lane'],
                                    road_ID = other_veh_info[i]['current_road'],
    ...
    def set_own_car(self, x, y, v, a):
        ...
        traci.vehicle.moveToXY('ego', '-gneE18', 0, self.__own_x +
                               self.egocar_length / 2 *
                               math.cos(math.radians(self.__own_a)),
                               self.__own_y + self.egocar_length / 2 *
        ...
        for veh in self.random_traffic:
            ...
            traci.vehicle.addLegacy(vehID=veh,
                                    ...
                                    typeID=(self.random_traffic[veh]
                                            [traci.constants.VAR_TYPE]))
            # print('edgeList',len(self.random_traffic[veh][87]) )
            # print('edgeList',self.random_traffic[veh][87])
...
if __name__ == "__main__":
    ...
    # print(__file__)
    traci.start([sumoBinary, "-c", "Map/Map3_Highway_v3/traffic_generation_Vehicle Only Traffic_Dense.sumocfg",
                 "--step-length", "1"])
